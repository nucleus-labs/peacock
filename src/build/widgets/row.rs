...
pub fn compose(node: NodeAsync, widget_collection: &crate::build::WidgetCollection) -> WidgetContext {
    ...
    let content: String = if node_guard.children.is_empty() {
        ...
        node_guard.children.iter()
            .map(|child| WidgetContext::from_node(child.clone(), widget_collection).gen_constructor())
            .collect::<Vec<_>>()
            .join(", ")
    };
    
    let construction: String = match id.clone() {
        Some(_id) => format!("Box::new(widgets::BuilderRow::new(Some(\"{_id}\"), vec![{}]))", content),
        None => format!("Box::new(widgets::BuilderRow::new(None, vec![{}]))", content),
    };
